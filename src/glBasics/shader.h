...
public:
    ...
    bool loadCompute(const char* computePath, const char* wrkGrpReplaceString = nullptr, glm::uvec3 newWrkGrpSize = glm::vec4())
    {
        std::string computeCode;

        try
        {
            computeCode = openAndReadFile(computePath);
            findAndIncludeFiles(computeCode);
            std::stringstream ss;
            ss << "local_size_x = " << newWrkGrpSize.x << ", ";
            ss << "local_size_y = " << newWrkGrpSize.y << ", ";
            ss << "local_size_z = " << newWrkGrpSize.z;
            if (wrkGrpReplaceString != nullptr)
            {
                replacePlaceholder(computeCode, std::string(wrkGrpReplaceString), ss.str());
            }
        }
...
private:
    ...
    void replace(std::string& original, const std::string& searchText, const std::string& replaceText)
    {
        size_t pos = 0u;
    ...
    template<typename T>
    void replacePlaceholder(std::string& code, std::string searchString, T newText)
    {
        replace(code, searchString, std::string(newText));
    }
